...
$myid = $mysqli->query("SELECT * FROM users WHERE id = ".$_SESSION['user_id']);
if($pok->num_rows < 1){ ?>
...
<p id=txt><b>&nbsp;&nbsp;Список атак: <?php $mysqli['id'] ?></b><br>
<table>
<? 
...
<center><h2>Покемон по вашему запросу не найден.</h2></center>
<?php 
return;
}elseif($pok->num_rows == 1){ 
     ...
     <table width='100%'>
       <tr>
         <td colspan='2' class="title">
           <table width='100%'>
            <tr>
             <td width=50><a href="pokedex.php?sp_id=<? print $id-1;?>"><<</a></td>
             <td align="center"><h2>#<? print numbPok($id).' '.$pok['name'];?></h2>
       ...
       <tr>
         <td valign='top' align='center' width='255'>
		   ...
		   <? if(empty($pok['type']) || $pok['type'] == 'not'){}else{?>
		   <img src='http://claimbe.ru/img/types/<?=$pok['type'];?>.gif' title='Тип - <?=atktip($pok['type']);?>'>
		   <?}?>
		   <? if(empty($pok['type_two']) || $pok['type_two'] == 'not'){}else{?>
         ...
         <td valign='top'>
           ...
           <?
               $pok_count= $mysqli->query("SELECT COUNT(id) as counts FROM user_pokemons WHERE basenum= '".$id."'")->fetch_assoc();
               $shine_pok_count= $mysqli->query("SELECT COUNT(id) as counts FROM user_pokemons WHERE type<>'normal' AND basenum= '".$id."'")->fetch_assoc();
           ?>
           <b>Всего в игре: <?=$pok_count['counts'];?>/<?=$shine_pok_count['counts'];?></b>
           
           <p><b>Базовые статы:</b><br>
           <table>
             <tr><td>&nbsp;&nbsp;НР:</td><td><?=$pok['hp'];?></td></tr>
             <tr><td>&nbsp;&nbsp;Атака:</td><td><?=$pok['atk'];?></td></tr>
             <tr><td>&nbsp;&nbsp;Защита:</td><td><?=$pok['def'];?></td></tr>
             <tr><td>&nbsp;&nbsp;Скорость:</td><td><?=$pok['spd'];?></td></tr>
             <tr><td>&nbsp;&nbsp;Спец.Атака:</td><td><?=$pok['satk'];?></td></tr>
		     <tr><td>&nbsp;&nbsp;Спец.Защита:</td><td><?=$pok['sdef'];?></td></tr>
		   </table>
         </td>
       </tr>
       <tr>
       ...
       <tr>
	   <?
	   $pok_attacks = $mysqli->query("SELECT * FROM attack_learn WHERE poke_base_id = ".$id);
	   ?>
         <td colspan='2'>
           <table width=100%>
		     <td width=33% valign=top><table><tr class='title'><td width='45'>&nbsp;</td><td>Атаки:</td></tr>
			 <?
			 while($pok_attack = $pok_attacks->fetch_assoc()){
				 ...
				 ?>
			 <tr><td><?=$pok_attack['atc_lvl'];?> ур.</td><td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?=$pok_attack['atac_id'];?>','at','width=500,height=230');><img src="http://claimbe.ru/img/question.gif" alt='Инфо' border='0'></a>  <?=$name['attac_name_eng'];?></td></tr>
			 <?} ?>
			 </table></td>
		    ...
		    <img src=http://claimbe.ru/img/question.gif alt=Инфо border=0></a>  <?=$nameN['attac_name_eng'];?></td></tr>
             <?php } ?>
