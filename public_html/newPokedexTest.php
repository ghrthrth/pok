...
if (isset($_GET['s'])) {
    if (is_numeric($_GET['s'])) {
        ...
        numbPok($search);
        die("<script>location.href='newPokedexTest.php?sp_id=" . $search . "';</script>");
    } elseif (is_string($_GET['s'])) {
        $search = obTxt($_GET['s']);
        $sql = "SELECT * FROM base_pokemon WHERE name LIKE '%" . $search . "%'";
        ...
        if ($pok->num_rows < 1) { ?>
            ...
            <STYLE>
                #searchfield {
                    ...
                    color: #99bfe7;
                    background: #AFD0F1 url(http://oldpokemon.ru/img/search-icon.gif) 0 -23px no-repeat;
                    margin: 0;
                }
                ...
                #searchfield:focus,
                #searchfield:focus {
                    background: #B8D5F1 url(http://oldpokemon.ru/img/search-icon.gif) 0 -88px no-repeat;
                    color: #1E3955;
                }
            </STYLE>

            <form action="newPokedexTest.php" method="GET" style="margin:0">
        ...
        <?php
            ...
            die("<script>location.href='newPokedexTest.php?sp_id=" . $pok['id'] . "';</script>");
        } else { ?>
            <LINK REL="Stylesheet" HREF="style.css" TYPE="text/css">
            <STYLE>
                #searchfield {
    ...
    <?
            while ($p = $pok->fetch_assoc()) {
                echo "<tr><td><img src='img/pkmna/" . numbPok($p['id']) . ".gif' border=0></td><td align=center><a href='newPokedexTest.php?sp_id=" . $p['id'] . "'>#" . numbPok($p['id']) . " " . $p['name'] . "</a></td></tr>";
            }
            echo "</table>";
            exit();
        }
        ...
        Header("Location: newPokedexTest.php");
    }
}
        ...
        <table width='100%'>
            <tr>
                <td colspan='2' class="title">
                    <table width='100%'>
                        <tr>
                            <td width=50><a href="newPokedexTest.php?sp_id=<? print $id - 1; ?>">
                                    <<< /a>
                            </td>
                            <td align="center">
                                <h2>#<? print numbPok($id) . ' ' . $pok['name']; ?></h2>
                            <td width=50 align='right'><a href='newPokedexTest.php?sp_id=<? print $id + 1; ?>'>>></a></td>
                        </tr>
                    </table>
                </td>
            </tr>
            <tr>
                <td valign='top' align='center' width='255'>
                    <img src='http://oldpokemon.ru/img/pkmn/<?= $img; ?>/<?= numbPok($id); ?>.jpg' width=250 height=190 border=1>
                    <br>
                    <? if (!isset($_GET['shine'])) { ?>
                        <b>обычный</b> -
                        <a href='newPokedexTest.php?sp_id=<?= numbPok($id); ?>&shine'>шайни</a>
                    <? } else { ?>
                        <a href='newPokedexTest.php?sp_id=<?= numbPok($id); ?>'>обычный</a> -
                        <b>шайни</b>
                    <? } ?>
                    <div title='Категория силы: <?= $pok['power_category']; ?>' style='width:255; height:18; text-align:left; margin-top:-210'>&nbsp;&nbsp;<? for ($a = 0; $a < $pok['power_category']; $a++) { ?><img src="http://oldpokemon.ru/img/star.png" border='0'><? } ?></div>
                    <div style='width:255; height:30; text-align:right; margin-top:160'>
                        <? if (empty($pok['type']) || $pok['type'] == 'not') {
                        } else { ?>
                            <img src='http://oldpokemon.ru/img/types/<?= $pok['type']; ?>.gif' title='Тип - <?= atktip($pok['type']); ?>'>
                        <? } ?>
                        <? if (empty($pok['type_two']) || $pok['type_two'] == 'not') {
                        } else { ?>
                            <img src='http://oldpokemon.ru/img/types/<?= $pok['type_two']; ?>.gif' title='Тип - <?= atktip($pok['type_two']); ?>'>&nbsp;&nbsp;
                        <? } ?>
                    </div>
                </td>
                <td valign='top'>
                    ...
                    <b><img src='http://oldpokemon.ru/img/sex_0.gif'> / <img src='http://oldpokemon.ru/img/sex_1.gif'> :</b> <?= $pok['sex_m']; ?>/<?= $pok['sex_f']; ?><br>
                    <b>Редкость:</b> 1<br>
            ...
            <tr>
                <td colspan='2'>
                    <?= $pok['about']; ?><p>
                        ...
                        if ($shd['counts'] != 0) {
                            echo "<br><br><b>Нападает на локациях:</b><br>";
            ...
            <tr>
                <?
                $pok_attacks_newExplode = $mysqli->query("SELECT * FROM base_pokemon_form WHERE pok = '" . $id . "'")->fetch_assoc();

                $attackExplode = explode(',', $pok_attacks_newExplode['atks']);
                $lvlExplode = explode(',', $pok_attacks_newExplode['lvl']);




                $mergeAr = array_combine($attackExplode, $lvlExplode);
                    ...
                    <table width=100%>
                        <td width=33% valign=top>
                            <table>
                                ...
                                // print_r($lvlExplode);
                                // echo '</pre>';
                                foreach ($mergeAr as $attckKey => $lvlVal) {
                                    $name = $mysqli->query("SELECT * FROM base_attacks WHERE atac_id = " . $attckKey)->fetch_assoc();
                                ?>
                                    <tr>
                                        <td><?= $lvlVal; ?> ур.</td>
                                        <td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?= $attckKey; ?>','at','width=500,height=230');><img src="http://oldpokemon.ru/img/question.gif" alt='Инфо' border='0'></a> <?= $name['attac_name_eng']; ?></td>
                                    </tr>
                                <? } ?>
                            </table>
                        </td>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?= $tms["attack"]; ?>','at','width=500,height=230');>
                                                <img src=http://oldpokemon.ru/img/question.gif alt=Инфо border=0></a> <?= $nameN['attac_name_eng']; ?></td>
                                    </tr>
                                <?php } ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?= $egga["attack_id"]; ?>','at','width=500,height=230');>
                                                <img src=http://oldpokemon.ru/img/question.gif alt=Инфо border=0></a> <?= $nameEGG['attac_name_eng']; ?></td>
                                    </tr>
                                <?php } ?>
