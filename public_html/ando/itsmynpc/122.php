...
if($npc == 122){
if(info_quest(117,'step') == 6){
	$textNPC = 'На вас нападает Шарпедо!';
		$inCatchPok = $mysqli->query("select * from `base_pokemon` WHERE `id`='319' LIMIT 1")->fetch_assoc();
		...
		$atk_pok1 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '319' and `atc_lvl` < '$lvl' ORDER BY RAND() LIMIT 1")->fetch_assoc();
		$atk_pok2 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '319' and `atc_lvl` < '$lvl' and `atac_id` != '".$atk_pok1['atac_id']."' ORDER BY RAND() LIMIT 1")->fetch_assoc();
		$atk1 = $atk_pok1['atac_id'];
		$atk2 = $atk_pok2['atac_id'];
		$mysqli->query("INSERT INTO pokem_vs (basenum,name,lvl,hp,hp_max,attack,def,speed,sp_attack,sp_def,har,type,gender,atk1,atk2,money,catch ) VALUES('319','Sharpedo','$lvl','$hp','$hp','$atk','$def','$speed','$satk','$sdef','$harR','normal','1','$atk1','$atk2','100','1')"); 
