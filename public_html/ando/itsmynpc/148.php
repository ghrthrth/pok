...
items[1][0]=1;
items[1][1]=309;
items[1][2]='Ванильная конфета';
items[1][3]=30000;
items[1][4]=-1;
items[1][5] = new Image(32,32);
items[1][5].src='img/items/309.gif';
items[2] = new Array(5);
items[2][0]=2;
items[2][1]=1011;
...
sell_items_amount=9;currency_txt=' кр.';   buys = new Array(sell_items_amount);
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=148&answ_id=1 method=post>
...
<?php    }else{
  $info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
    function price_item($itm) { //цены
             if ($itm == 330) { $prc = 400000; }
        else if ($itm == 1011) { $prc = 6000000; }
        else if ($itm == 1012) { $prc = 1500000; }
        else if ($itm == 1005) { $prc = 1500000; }
        else if ($itm == 1025) { $prc = 2000000; }
        else if ($itm == 1004) { $prc = 3000000; }
        else if ($itm == 1019) { $prc = 6000000; }
        else if ($itm == 1015) { $prc = 2000000; }
        else if ($itm == 1003) { $prc = 5000000; }
        else if ($itm == 309) { $prc = 30000; }
        else { return 0; }
    return $prc;
  }
