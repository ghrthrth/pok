...
items[1][5].src='img/items/26.gif';
items[2] = new Array(5);
items[2][0]=2;
items[2][1]=248;
items[2][2]='Зелье памяти';
items[2][3]=600000;
items[2][4]=-1;
items[2][5] = new Image(32,32);
items[2][5].src='img/items/248.gif';
items[3] = new Array(5);
items[3][0]=5;
items[3][1]=286;
items[3][2]='Крепкий кофе';
items[3][3]=70000;
items[3][4]=-1;
items[3][5] = new Image(32,32);
items[3][5].src='img/items/286.gif';
items[4] = new Array(5);
items[4][0]=6;
items[4][1]=306;
items[4][2]='Режущий клык';
items[4][3]=3000000;
items[4][4]=-1;
items[4][5] = new Image(32,32);
items[4][5].src='img/items/306.gif';
items[5] = new Array(5);
items[5][0]=7;
items[5][1]=291;
items[5][2]='Амурит';
...
items[5][5].src='img/items/291.gif';
items[6] = new Array(5);
items[6][0]=8;
items[6][1]=42;
items[6][2]='Скобовое кольцо';
items[6][3]=600000;
items[6][4]=-1;
items[6][5] = new Image(32,32);
items[6][5].src='img/items/42.gif';
items[7] = new Array(5);
items[7][0]=9;
items[7][1]=438;
items[7][2]='Пакт о ненападении';
items[7][3]=300000;
items[7][4]=-1;
items[7][5] = new Image(32,32);
items[7][5].src='img/items/438.gif';
...
<span id=txt>
<h1>Барыга</h1>
<div><big><b>Ваш баланс: <?=price($info['count']);?> Кредитов</b></big></div><TABLE border=0><TR valign=top><TD width=300>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=133&answ_id=1 method=post>
...
<?php    }else{
  $info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
    function price_item($itm) {
             if ($itm == 26) { $prc = 2000000; }
        else if ($itm == 248) { $prc = 600000; }
        else if ($itm == 286) { $prc = 70000; }
        else if ($itm == 306) { $prc = 3000000; }
        else if ($itm == 291)  { $prc = 40000; }
        else if ($itm == 42)  { $prc = 600000; }
        else if ($itm == 438)  { $prc = 300000; }
        else { return false; }
    return $prc;
  }
