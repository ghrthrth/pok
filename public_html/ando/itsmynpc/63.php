...
$nameNPC = 'Кассир';
if($npc == 63){ }else{  die("<script>parent._location.location='game.php?fun=m_location&map=1';</script>"); }
if(array_key_exists('subm',$_POST)){
	
		function price_item($itm){
		if($itm == 72) { $prc = 2; }else
		if($itm == 1053) { $prc = 5; }else
		if($itm == 43) { $prc = 15; }else
		if($itm == 1012) { $prc = 35; }else
		if($itm == 139) { $prc = 45; }else
		if($itm == 1004) { $prc = 50; }else
		if($itm == 251) { $prc = 60; }else
		if($itm == 1011) { $prc = 70; }
        ...
        plus_item($sell_count,$sell_id,$user_id);
            ...
            $mysqli->query("UPDATE `users` SET `jeton` = `jeton`-'$sell' WHERE `id` = '".$user_id."'");
			$textNPC = 'Предмет удачно приобретен!';
        }else{
			$textNPC = 'Недостаточно жетонов!';
        }
}else
if(empty($answer)){
?>
...
items[2][0]=1;
items[2][1]=1053;
items[2][2]='Случайная карта';
items[2][3]=5;
items[2][4]=-1;
items[2][5] = new Image(32,32);
items[2][5].src='img/items/1053.gif';
items[3] = new Array(5);
items[3][0]=1;
items[3][1]=43;
items[3][2]='Линзы';
...
items[3][5].src='img/items/43.gif';
items[4] = new Array(5);
items[4][0]=1;
items[4][1]=1012;
items[4][2]='TM 12 - Taunt';
items[4][3]=35;
items[4][4]=-1;
...
items[5][0]=1;
items[5][1]=139;
items[5][2]='Чешуя дракона';
items[5][3]=45;
items[5][4]=-1;
items[5][5] = new Image(32,32);
items[5][5].src='img/items/139.gif';
items[6] = new Array(5);
items[6][0]=1;
items[6][1]=1004;
...
items[7][0]=1;
items[7][1]=251;
items[7][2]='Овальный камень';
items[7][3]=60;
items[7][4]=-1;
items[7][5] = new Image(32,32);
items[7][5].src='img/items/251.gif';
items[8] = new Array(5);
items[8][0]=1;
items[8][1]=1011;
items[8][2]='TM 6 - Toxic';
items[8][3]=70;
items[8][4]=-1;
items[8][5] = new Image(32,32);
items[8][5].src='img/items/1011.gif';



		sell_items_amount=8;currency_txt=' жет.';   buys = new Array(sell_items_amount);
...
<span id='txt'>
<h1>Кассир</h1>
<div><big><b>Ваш баланс: <?=price($user['jeton']);?> жет.</b></big></div><TABLE border='0'><TR valign='top'><TD width='300'>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=63&answ_id=1' method='post'>
