...
$nameNPC = 'Кассир';
if(array_key_exists('subm',$_POST)){ // проверка wterh
	$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
		function price_item($itm){
		if($itm == 192) { $prc = 700000; }else
		//if($itm == 192) { $prc = 500000; }else
	    {return 0;}
		return $prc;
	}
...
}elseif($npc == 138 && empty($answer)){
	$textNPC = 'Добро пожаловать в Аэровокзал региона Хоэнна. Давайте сюда ваш багаж. Ваших покемонов вы должны строго держать в покеболах во время всего полёта. Проходим регистрацию?';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Да</a></li>';
}elseif($npc == 138 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][0]=1;
items[1][1]=192;
items[1][2]='Билет на дирижабль Джото - Хоэнн';
items[1][3]=700000;
items[1][4]=-1;
items[1][5] = new Image(32,32);
items[1][5].src='img/items/192.gif';
/*items[2] = new Array(5);
items[2][0]=2;
items[2][1]=192;
items[2][2]='Билет на катер Джото - Хоэнн';
items[2][3]=500000;
items[2][4]=-1;
items[2][5] = new Image(32,32);
items[2][5].src='img/items/192.gif';*/
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=138&answ_id=1' method='post'>
