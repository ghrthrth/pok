...
if($npc == 57){ }else{  die("<script>parent._location.location='game.php?fun=m_location&map=1';</script>"); }
...
if(array_key_exists('subm',$_POST)){ // проверка wterh
		...
		if($itm == 312) { $prc = 10000; }else
		if($itm == 313) { $prc = 60000; }
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=57&answ_id=1' method='post'>
