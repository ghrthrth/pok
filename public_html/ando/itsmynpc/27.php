...
if(isset($_POST['subm'])){
	$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
		...
		if($itm == 257) { $prc = 6950000; }
		if($itm == 258) { $prc = 270000; }
		if($itm == 441) { $prc = 1000; }
		else{return 0;}
		return $prc;
	}
        ...
        minus_item($sell,1,$user_id);
			$textNPC = 'Предметы успешно куплены!';
        }else{
			$textNPC = 'Недостаточно денег!';
        }
}elseif($npc == 27 && empty($answer)){
	$textNPC = 'Добрый день я занимаюсь координацией игровых кланов. Вы можете приобрести у меня Клан-ордер, необходимый для создания клана. А так же Эмблемную книгу, для увеличения количества эмблем вашего клана';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Покажите ваши товары.</a></li>';
}elseif($npc == 27 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][0]=1;
items[1][1]=257;
items[1][2]='Клан-ордер';
items[1][3]=6950000;
items[1][4]=-1;
items[1][5] = new Image(32,32);
items[1][5].src='img/items/257.gif';
        items[2] = new Array(5);
items[2][0]=1;
items[2][1]=258;
items[2][2]='Эмблемная книга';
items[2][3]=270000;
items[2][4]=-1;
items[2][5] = new Image(32,32);
items[2][5].src='img/items/258.gif';
        items[3] = new Array(5);
items[3][0]=1;
items[3][1]=441;
items[3][2]='Ордер захвата локации';
items[3][3]=1000;
items[3][4]=-1;
items[3][5] = new Image(32,32);
items[3][5].src='img/items/441.gif';
		sell_items_amount=3;currency_txt=' кр.';   buys = new Array(sell_items_amount);
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=27&answ_id=1' method='post'>
