...
$mypokoff = $mysqli->query('SELECT * FROM user_pokemons WHERE user_id = '.$_SESSION['user_id'].' AND active = 0 ORDER BY basenum ');
...
<span id='txt'>
<h1>Покецентр</h1>
<h2>Центральный питомник Лиги-17</h2>
...
if(isset($_POST['to_name']))
{
    ...
    if($findstr != "")
    {
        $mypokoff = $mysqli->query("SELECT * FROM user_pokemons WHERE user_id = '".$_SESSION['user_id']."'".$findstr." AND active = 0 ORDER BY basenum ");
    }
}
?>
...
<form name='find' action='game.php?fun=m_npc&npc_id=8&answ_id=2&map=1' method='post'>      
...
<li><a href='game.php?fun=m_npc&npc_id=8&answ_id=2&map=1&TPI_id=<?=$off['id'];?>'>Активировать<?php  print 

$name = '<a href=javascript: title=Информация onclick=window.open("game.php?fun=anus&p_id=' . $off['basenum'] . '&trn=' . $_SESSION['user_id'] . '","pokeinf","fullscreen=no,scrollbars=yes,width=520,height=250");><img src="https://claimbe.ru/img/pkmna/' . numbPok($off['basenum']) . '.gif"> #' . numbPok($off['basenum']) . ' ' . $off['name_new'] . ' ' . $off['lvl'] . '-lvl</a>';
numbPok($off['basenum']).' '.$pokname.' '.$off['lvl'].'-lvl';?><?php ?></a>&nbsp;&nbsp;&nbsp;<span style='color:red; font-size:11px'><?=$off['type'];?></span></b>, <span style='color:#1255d1; font-size:11px'><?=$off['sex'];?><b><img src='img/sex_<?=$sex;?>.gif' width=7 height=13 border=0> <?=$pok_base['sex'];?></span><span style='color:#1255d1; font-size:10px'>HP<?=$off['hp_gen'];?> </span><span style='color:#1255d1; font-size:10px'>|<font size="1" color="<?if($har['atk']==1.1){print "green";}elseif($har['atk']==0.9){print "red";}else{print "blue";};?>" face="Arial">A<?=$off['atc_gen'];?></font> </span><span style='color:#1255d1; font-size:10px'>|<font size="1" color="<?if($har['def']==1.1){print "green";}elseif($har['def']==0.9){print "red";}else{print "blue";};?>" face="Arial">D<?=$off['def_gen'];?></font> </span><span style='color:#1255d1; font-size:10px'>|<font size="1" color="<?if($har['speed']==1.1){print "green";}elseif($har['speed']==0.9){print "red";}else{print "blue";};?>" face="Arial">S<?=$off['speed_gen'];?></font> </span><span style='color:#1255d1; font-size:10px'>|<font size="1" color="<?if($har['satk']==1.1){print "green";}elseif($har['satk']==0.9){print "red";}else{print "blue";};?>" face="Arial">SA<?=$off['spatc_gen'];?></font> </span><span style='color:#1255d1; font-size:10px'>|<font size="1" color="<?if($har['sdef']==1.1){print "green";}elseif($har['sdef']==0.9){print "red";}else{print "blue";};?>" face="Arial">SD<?=$off['spdef_gen'];?></font></span> <?if($off['egg_attack_id'] > 0 AND $off['atk4'] == $off['egg_attack_id']){?>Я.а: <?=$off['egg_attack']?><?}?><BR>
<?php 
...
<?php  if(!empty($_GET['TPI_id'])){ 
	...
	$zapros = $mysqli->query("SELECT * FROM user_pokemons WHERE id = ".$id)->fetch_assoc();
	if($zapros['user_id'] != $_SESSION['user_id']) echo "<script>location.href='".$_SERVER['HTTP_REFERER']."';</script>";
...
elseif($zapros['gender'] == 'female' ) $sex = '1' ;
	         ...
	         <p>&nbsp;</p><p></p><div align="right" style="color:#82b1e2">ID: <?=$id;?></div>
	      	...
	      	<?php 
				      ...
				      <div ID='moves0' class='poke' style="top:-400px;">
				         ...
				         &nbsp;<br><b></b></div>
					<?php 	$owner = $mysqli->query('SELECT * FROM users WHERE id = '.$zapros['owner'])->fetch_assoc();
						$user_data = $mysqli->query('SELECT * FROM users WHERE id = '.$zapros['user_id'])->fetch_assoc(); ?>
	...
	<b><a href="game.php?fun=p_base&amp;map=1&amp;type=extract&amp;TP_id=<?=$id;?>">&lt;Активировать&gt;</a></b></td>
<?php }?>
<?php  }else{
