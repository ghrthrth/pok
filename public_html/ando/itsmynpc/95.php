...
}elseif($npc == 95 && empty($answer)){
	$textNPC = 'Добрый день, я продаю билеты до Джото. Тебя интересует?';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Да</a></li>';
}elseif($npc == 95 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][0]=1;
items[1][1]=192;
items[1][2]='Билет на корабль Джото - о. Селен';
items[1][3]=500000;
items[1][4]=-1;
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=95&answ_id=1' method='post'>
