...
if($npc == 34){ }else{  die("<script>parent._location.location='game.php?fun=m_location&map=1';</script>"); }
if(!$answer){
$textNPC = 'Все было так быстро! Мы и опомниться не успели, как эти варишки украли все наши диски для записи! Думаю что лучший способ отблагодарить тренеров за найденые диски, уже готовыми ТМ-Атаками!';
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=34&answ_id=1 method=post>
