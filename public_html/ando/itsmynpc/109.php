...
$tur = 1;
if($npc == 109){ }else{  die("<script>parent._location.location='game.php?fun=m_location&map=1';</script>"); }
if($tur == 0){
if(empty($answer)){
$textNPC = 'Привет!';
}
...
if(array_key_exists('go',$_GET)) {
	$mysqli->query("UPDATE users SET location = '137', arena = '69' WHERE `id` = '".$_SESSION['user_id']."'");
	minus_item (3000,1);
