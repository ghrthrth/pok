...
}elseif($npc == 137 && empty($answer)){
	$textNPC = 'Добро пожаловать в Аэровокзал региона Джото. Давайте сюда ваш багаж. Ваших покемонов вы должны строго держать в покеболах во время всего полёта. Проходим регистрацию?';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Да</a></li>';
}elseif($npc == 137 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=137&answ_id=1' method='post'>
