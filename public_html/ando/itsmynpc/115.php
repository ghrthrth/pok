...
if($npc == 115){
    if($npc == 115 && empty($answer)){
		$textNPC = 'Вы хотел сорвать ветку с дерева, но перед вами выпрыгнул Судовудо!';
		$inCatchPok = $mysqli->query("select * from `base_pokemon` WHERE `id`='185' LIMIT 1")->fetch_assoc();
		...
		$atk_pok1 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '185' and `atc_lvl` < '$lvl' ORDER BY RAND() LIMIT 1")->fetch_assoc();
		$atk_pok2 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '185' and `atc_lvl` < '$lvl' and `atac_id` != '".$atk_pok1['atac_id']."' ORDER BY RAND() LIMIT 1")->fetch_assoc();
		$atk1 = $atk_pok1['atac_id'];
		$atk2 = $atk_pok2['atac_id'];
		$mysqli->query("INSERT INTO pokem_vs (basenum,name,lvl,hp,hp_max,attack,def,speed,sp_attack,sp_def,har,type,gender,atk1,atk2,money,item,catch ) VALUES('185','Sudowoodo','$lvl','$hp','$hp','$atk','$def','$speed','$satk','$sdef','$harR','normal','1','$atk1','$atk2','100','344','1')");
