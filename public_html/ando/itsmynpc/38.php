...
$nameNPC = 'Продавец в синем';
$baseloc['name'] = false;
if($npc == 38 && empty($answer)){
...
items[1][5].src='img/items/1013.gif';
items[2] = new Array(5);
items[2][0]=2;
items[2][1]=1000;
items[2][2]='TM 15 - Hyper Beam';
items[2][3]=1000000;
items[2][4]=-1;
...
items[3][5].src='img/items/1014.gif';
items[4] = new Array(5);
items[4][0]=6;
items[4][1]=1015;
items[4][2]='TM 31 - Brick Break';
items[4][3]=700000;
items[4][4]=-1;
items[4][5] = new Image(32,32);
items[4][5].src='img/items/1015.gif';
items[5] = new Array(5);
items[5][0]=7;
items[5][1]=1016;
items[5][2]='TM 43 - Secret Power';
items[5][3]=400000;
items[5][4]=-1;
items[5][5] = new Image(32,32);
items[5][5].src='img/items/1016.gif';
items[6] = new Array(5);
items[6][0]=8;
items[6][1]=1017;
items[6][2]='TM 10 - Hidden Power';
items[6][3]=500000;
items[6][4]=-1;
items[6][5] = new Image(32,32);
items[6][5].src='img/items/1017.gif';
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=38&answ_id=1 method=post>
...
<?php   }else{
  ...
  function price_item($itm){
    if($itm == 1013) { $prc = 300000; }else
    if($itm == 1000) { $prc = 1000000; }else
    if($itm == 1014) { $prc = 600000; }else
    if($itm == 1015) { $prc = 700000; }else
    if($itm == 1016) { $prc = 400000; }else
    if($itm == 1017) { $prc = 500000; }else{
        return 0;
    }
    return $prc;
  }
