...
if($npc == 22){
	if(info_quest(4,'step') == 5){
		$nameNPC = 'Сквиртд';
		$textNPC = '<i>Сквирл...</i>';
	}elseif(item_isset(104,1)){
		$nameNPC = 'Сквиртл';
		$textNPC = '<i>Сквирл...</i>';
		quest_update(4,3);
	}else{
		$inCatchPok = $mysqli->query("select * from `base_pokemon` WHERE `id`='73' LIMIT 1")->fetch_assoc();
		...
		$mysqli->query("INSERT INTO pokem_vs (basenum,name,lvl,hp,hp_max,attack,def,speed,sp_attack,sp_def,har,type,gender,atk1,atk2,money,item ) VALUES('73','Tentacruel','$lvl','$hp','$hp','$atk','$def','$speed','$satk','$sdef','$harR','normal','1','$atk1','$atk2','100','104')"); 
