...
items[4][0]=6;
items[4][1]=307;
items[4][2]='Кусок ткани';
items[4][3]=3000000;
items[4][4]=-1;
items[4][5] = new Image(32,32);
items[4][5].src='img/items/307.gif';
items[5] = new Array(5);
items[5][0]=7;
items[5][1]=383;
items[5][2]='Украденный секретный ящик';
items[5][3]=160000;
items[5][4]=-1;
items[5][5] = new Image(32,32);
items[5][5].src='img/items/383.gif';
items[6] = new Array(5);
items[6][0]=8;
items[6][1]=42;
...
items[7][5].src='img/items/28.gif';
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=126&answ_id=1 method=post>
...
<?php    }else{
  $info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
    function price_item($itm) {
             if ($itm == 312) { $prc = 20000; }
        else if ($itm == 313) { $prc = 40000; }
        else if ($itm == 286) { $prc = 70000; }
        else if ($itm == 307) { $prc = 3000000; }
        else if ($itm == 383)  { $prc = 160000; }
        else if ($itm == 42)  { $prc = 600000; }
        else if ($itm == 28)  { $prc = 1500000; }
        else { return 0; }
    return $prc;
  }
