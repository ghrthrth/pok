...
$nameNPC = 'Воришка';
$baseloc['name'] = false;
if($npc == 72 && empty($answer)){
$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1034'")->fetch_assoc();
?>
<html>
  <head>
...
items[1][5].src='img/items/72.gif';
items[2] = new Array(5);
items[2][0]=2;
items[2][1]=43;
items[2][2]='Линзы';
...
items[2][5].src='img/items/43.gif';
items[3] = new Array(5);
items[3][0]=5;
items[3][1]=49;
items[3][2]='Обрывок заклинаний';
items[3][3]=30;
items[3][4]=-1;
items[3][5] = new Image(32,32);
items[3][5].src='img/items/49.gif';
items[4] = new Array(5);
items[4][0]=6;
items[4][1]=45;
items[4][2]='Белый колокольчик';
items[4][3]=30;
items[4][4]=-1;
items[4][5] = new Image(32,32);
items[4][5].src='img/items/45.gif';
items[5] = new Array(5);
items[5][0]=7;
items[5][1]=1006;
items[5][2]='TM 35 - Flamethrower';
items[5][3]=70;
items[5][4]=-1;
items[5][5] = new Image(32,32);
items[5][5].src='img/items/1006.gif';
items[6] = new Array(5);
items[6][0]=8;
items[6][1]=1002;
items[6][2]='TM 30 - Shadow Ball';
items[6][3]=70;
items[6][4]=-1;
items[6][5] = new Image(32,32);
items[6][5].src='img/items/1002.gif';
items[7] = new Array(5);
items[7][0]=32;
items[7][1]=304;
items[7][2]='Электрайзер';
items[7][3]=100;
items[7][4]=-1;
items[7][5] = new Image(32,32);
items[7][5].src='img/items/304.gif';
items[8] = new Array(5);
items[8][0]=8;
items[8][1]=305;
items[8][2]='Магмарайзер';
items[8][3]=100;
items[8][4]=-1;
items[8][5] = new Image(32,32);
items[8][5].src='img/items/305.gif';
items[9] = new Array(5);
items[9][0]=9;
items[9][1]=1024;
items[9][2]='TM 34 Shock Wave';
items[9][3]=150;
items[9][4]=-1;
items[9][5] = new Image(32,32);
items[9][5].src='img/items/1006.gif';


sell_items_amount=9;currency_txt=' Грибов';   buys = new Array(sell_items_amount);
...
<span id=txt>
<h1>Пятый этаж</h1>
<div><big><b>Ваш баланс: <?=price($info['count']);?> Грибов</b></big></div><TABLE border=0><TR valign=top><TD width=300>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=72&answ_id=1 method=post>
...
<?php   }else{
  ...
  function price_item($itm){
    if($itm == 72) { $prc = 2; }else
    if($itm == 43) { $prc = 20; }else
    if($itm == 49) { $prc = 30; }else
    if($itm == 45) { $prc = 30; }else
    if($itm == 1006) { $prc = 70; }else
    if($itm == 1002) { $prc = 70; }else
    if($itm == 304) { $prc = 100; }else
    if($itm == 305) { $prc = 100; }else
