...
$nameNPC = 'Томми';
$baseloc['name'] = false;
if($npc == 4001 && empty($answer)){
$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1029'")->fetch_assoc();
?>
<html>
	<head>
...
items[1][5].src='img/items/29.gif';
items[2] = new Array(5);
items[2][0]=2;
items[2][1]=309;
items[2][2]='Ванильная Конфета';
items[2][3]=2;
items[2][4]=-1;
items[2][5] = new Image(32,32);
items[2][5].src='img/items/309.gif';
items[3] = new Array(5);
items[3][0]=3;
items[3][1]=291;
items[3][2]='Амурит';
...
items[3][5].src='img/items/291.gif';
items[4] = new Array(5);
items[4][0]=4;
items[4][1]=427;
items[4][2]='Случайная Карта';
items[4][3]=5;
items[4][4]=-1;
items[4][5] = new Image(32,32);
items[4][5].src='img/items/427.gif';
items[5] = new Array(5);
items[5][0]=5;
items[5][1]=330;
items[5][2]='Набор Тренировки';
items[5][3]=10;
items[5][4]=-1;
items[5][5] = new Image(32,32);
items[5][5].src='img/items/330.gif';
items[6] = new Array(5);
items[6][0]=6;
items[6][1]=8;
items[6][2]='Блестки';
...
items[7][5].src='img/items/43.gif';
items[8] = new Array(5);
items[8][0]=1;
items[8][1]=293;
items[8][2]='Лупа';
...
items[8][5].src='img/items/293.gif';
items[9] = new Array(5);
items[9][0]=9;
items[9][1]=45;
items[9][2]='Белый Колокольчик';
items[9][3]=15;
items[9][4]=-1;
items[9][5] = new Image(32,32);
items[9][5].src='img/items/45.gif';
items[10] = new Array(5);
items[10][0]=10;
items[10][1]=42;
items[10][2]='Скобовое Кольцо';
items[10][3]=20;
items[10][4]=-1;
items[10][5] = new Image(32,32);
items[10][5].src='img/items/42.gif';
items[11] = new Array(5);
items[11][0]=11;
items[11][1]=1002;
items[11][2]='TM 30 - Shadow Ball';
items[11][3]=30;
items[11][4]=-1;
items[11][5] = new Image(32,32);
items[11][5].src='img/items/1002.gif';
items[12] = new Array(5);
items[12][0]=12;
items[12][1]=1023;
items[12][2]='TM 40 - Aerial Ace';
items[12][3]=30;
items[12][4]=-1;
items[12][5] = new Image(32,32);
items[12][5].src='img/items/1023.gif';
items[13] = new Array(5);
items[13][0]=13;
items[13][1]=1024;
items[13][2]='ТM 34 - Shock Wave';
items[13][3]=30;
items[13][4]=-1;
items[13][5] = new Image(32,32);
items[13][5].src='img/items/1024.gif';
items[14] = new Array(5);
items[14][0]=14;
items[14][1]=1019;
items[14][2]='TM 26 - Earthquake';
items[14][3]=50;
items[14][4]=-1;
items[14][5] = new Image(32,32);
items[14][5].src='img/items/1019.gif';



sell_items_amount=14;currency_txt=' шар.';   buys = new Array(sell_items_amount);
...
<span id=txt>
<h1>Томми</h1>
<div><big><b>Ваш баланс: <?=price($info['count']);?> Шар.</b></big></div><TABLE border=0><TR valign=top><TD width=300>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=4001&answ_id=1 method=post>
...
<?php    }else{
  $info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1029'")->fetch_assoc();
    function price_item($itm) {
             if ($itm == 29) { $prc = 1; }
        else if ($itm == 309) { $prc = 2; }
        else if ($itm == 291) { $prc = 2; }
        else if ($itm == 427) { $prc = 5; }
        else if ($itm == 330)  { $prc = 10; }
        else if ($itm == 8)  { $prc = 12; }
        else if ($itm == 43) { $prc = 12; }
        else if ($itm == 293) { $prc = 12; }
        else if ($itm == 45) { $prc = 15; }
        else if ($itm == 42) { $prc = 20; }
        else if ($itm == 1002) { $prc = 30; }
        else if ($itm == 1023) { $prc = 30; }
        else if ($itm == 1024)  { $prc = 30; }
        else if ($itm == 1019) { $prc = 50; }
        else { return 0; }
    return $prc;
  }
    ...
    for ($x = 0; $x < $cnt; $x++) {
        ...
        plus_item($sell_count,$sell_id,$user_id);
         ...
         minus_item($sell,1029,$user_id);
         echo "Предметы успешно куплены!";
          }else{
            echo "Не хватает шаров!";
          } 
}
?>