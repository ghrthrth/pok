...
}elseif($npc == 155 && empty($answer)){
	$textNPC = 'Здравствуйте! Не интересует данный билетик? Он позволит вам сыграть в Колесо Фортуны в доме напротив.';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Хм, интересненько. Покажите.</a></li>';
}elseif($npc == 155 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][0]=1;
items[1][1]=1051;
items[1][2]='Билет удачи';
items[1][3]=4500000;
items[1][4]=-1;
items[1][5] = new Image(32,32);
items[1][5].src='img/items/1051.gif';
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=155&answ_id=1' method='post'>
