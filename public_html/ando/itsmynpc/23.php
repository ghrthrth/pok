...
if($npc == 23){
	if(info_quest(4,'step') == 5){
		$nameNPC = 'Чармандер';
		$textNPC = '<i>Чар...</i>';
	}elseif(item_isset(105,1)){
		$nameNPC = 'Чармандер';
		$textNPC = '<i>Чарм...</i>';
		quest_update(4,4);
	}else{
		$inCatchPok = $mysqli->query("select * from `base_pokemon` WHERE `id`='57' LIMIT 1")->fetch_assoc();
		...
		$atk_pok1 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '57' and `atc_lvl` < '$lvl' LIMIT 1")->fetch_assoc();
		$atk_pok2 = $mysqli->query("SELECT * FROM attack_learn WHERE `poke_base_id` = '57' and `atc_lvl` < '$lvl' and `atac_id` != '".$atk_pok1['atac_id']."' LIMIT 1")->fetch_assoc();
		$atk1 = $atk_pok1['atac_id'];
		$atk2 = $atk_pok2['atac_id'];
		$mysqli->query("INSERT INTO pokem_vs (basenum,name,lvl,hp,hp_max,attack,def,speed,sp_attack,sp_def,har,type,gender,atk1,atk2,money,item ) VALUES('57','Primeape','$lvl','$hp','$hp','$atk','$def','$speed','$satk','$sdef','$harR','normal','1','$atk1','$atk2','100','105')"); 
