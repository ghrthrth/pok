...
$nameNPC = 'Работник';
$baseloc['name'] = false;
if($npc == 79 && empty($answer)){
$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '38'")->fetch_assoc();
?>
<html>
	<head>
...
<span id=txt>
<h1>Покемаркет</h1>
<div><big><b>Ваш баланс: <?=price($info['count']);?> Стимуляторов.</b></big></div><TABLE border=0><TR valign=top><TD width=300>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*400); load_buy_list();">
</form>
</TD><TD width=300>
  <div ID=buy_list>
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=79&answ_id=1 method=post>
...
<?php 	}else{
	...
	function price_item($itm){
        ...
        plus_item($sell_count,$sell_id,$user_id);
         ...
         minus_item($sell,38,$user_id);
         echo "Вы выбросили предмет и получили награду!";
          }else{
          	echo "Нет достаточного количества предметов!";
          } 
?>