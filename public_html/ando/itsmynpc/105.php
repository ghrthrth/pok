...
if(isset($_POST['subm'])){
	$info = $mysqli->query("SELECT * FROM user_items WHERE `user_id` = '".$user_id."' and `item_id` = '1'")->fetch_assoc();
        ...
        minus_item($sell,1,$user_id);
			$textNPC = 'Держи Транзистор!';
        }else{
			$textNPC = 'Недостаточно денег!';
        } 
}elseif($npc == 105 && empty($answer)){
	$textNPC = 'Привет, ты вероятно пришел за новинками в мире технологий?';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Да, покажите пожалуйста.</a></li>';
}elseif($npc == 105 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][0]=1;
items[1][1]=1032;
items[1][2]='Транзистор';
items[1][3]=200000;
items[1][4]=-1;
items[1][5] = new Image(32,32);
items[1][5].src='img/items/1032.gif';
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=105&answ_id=1' method='post'>
