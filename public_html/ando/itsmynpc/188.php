...
$nameNPC = 'Джувия'; // поменять имя если не нравится
$prov_psydak = $mysqli->query('SELECT * FROM user_pokemons WHERE basenum = 54 AND lvl = 1 AND user_id = ' . $_SESSION['user_id'] . ' LIMIT 1 ')->fetch_assoc();
...
if ($npc == 188 && empty($answer)) { // придумать айди нпс
	if (!info_quest(188, 'step')) {
	...
	} elseif (info_quest(188, 'step') == 1 or info_quest(188, 'step') == 2) {
		$textNPC = '...?';
		$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id=' . $npc . '&answ_id=2&map=1">А вот и я! Добрый день, принимайте будущее селекции</a></li>';
		quest_update(188, 2);
		...
		$textNPC = 'Спасибо что помог мне, направляйся в Восточный Шафран.'; // текст после выполнения квеста если с ней снова говорить
	}
	...
	if (!info_quest(188, 'step')) {
		...
		quest_update(188, 4, 1);
	}
} elseif ($npc == 188 && $answer == 4) {
	if (!info_quest(188, 'step')) {
		$textNPC = 'Так вышло, что я увлёкся селекцией и пришёл к гипотезе того, что некоторые уникальные покемоны - результат скрещивания идеальных особей разного вида. Да, знаю, звучит странно, но тем не менее мне нужны:  Psyduck,Wooper,Surskit,Clamperl и Basculin но помни что мне нужны именно они и только 1 уровня.';
		$moveNPC = '<li id="txt"><a href="game.php?fun=m_location&map=1">Хорошо, я их принесу! Найдеюсь , это и правда будет что-то интересное.</a></li>';
		quest_update(188, 1);
	}
} else {
