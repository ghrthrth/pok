...
<a href="game.php?fun=m_location&map=1"><< уйти</a><P>
	...
	?>
    	 <tr class='bottom_dot'>
    	  <td width='300'><a HREF=javascript: onClick=win1=window.open("pokedex.php?sp_id=<?=$pok['basenum'];?>","dex","width=600,height=600,scrollbars=yes");><img src='img/pkmna/<?=numbPok($pok['basenum']);?>.gif' border='0' alt='Покедекс' title='Покедекс'></a><a href=javascript: title='Покекарта' onclick=window.open("game.php?fun=pokeinf&p_id=<?=$pok["id"];?>&master=1","pokeinf","fullscreen=no,scrollbars=yes,width=520,height=250");>#<?php  print numbPok($pok['basenum']).' '.$pokname.' '.$pok['lvl'].'-lvl';?> </a></td>
