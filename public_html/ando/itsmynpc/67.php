...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*1); load_buy_list();">
  ...
  <P>
  <div align="right">
  <form name=FormBuy action=game.php?fun=m_npc&npc_id=67&answ_id=1 method=post>
...
<?php   }else{
  ...
  function price_item($itm){
    if($itm == 10) { $prc = 75000; }else
    if($itm == 11) { $prc = 75000; }else
    if($itm == 23) { $prc = 75000; }else
    if($itm == 24) { $prc = 75000; }else
    if($itm == 39) { $prc = 75000; }else
    if($itm == 59) { $prc = 75000; }else
    {
        return 0;
    }
    return $prc;
