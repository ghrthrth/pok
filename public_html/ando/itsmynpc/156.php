...
}elseif($npc == 156 && empty($answer)){
	$textNPC = 'Добрый день. В нашем банке вы можете обменять Жемчуг на Кредиты в размере 1 к 100.000 . Вас интересует данная услуга?';
	$moveNPC = '<li id="txt"><a href="game.php?fun=m_npc&npc_id='.$npc.'&answ_id=1&map=1">Да, я хочу обменять Жемчуг.</a></li>';
}elseif($npc == 156 && $answer == 1){
	$baseloc['name'] = false;
	$nameNPC = false;
?>
<html>
...
items[1][5].src='img/items/1.gif';

		sell_items_amount=1;currency_txt=' жем.';   buys = new Array(sell_items_amount);
...
<span id='txt'>
<h1>Работник банка</h1>
<div><big><b>Ваш баланс: <?=price($info['count']);?> жем.</b></big></div><TABLE border='0'><TR valign='top'><TD width='300'>
...
<input type="button" value=">>" style="width:70" onclick="buys[document.buy.sellID.value]=buys[document.buy.sellID.value]*1+Math.round(document.buy.amount.value*100000); load_buy_list();">
</form>
</TD><TD width='300'>
  <div ID='buy_list'>
  ...
  <P>
  <div align="right">
  <form name='FormBuy' action='game.php?fun=m_npc&npc_id=156&answ_id=1' method='post'>
