...
if(isset($kom) and empty($battle)) {
    $kMy = $mysqli->query("SELECT * FROM team_btl WHERE `kom` = '".$kom['kom']."' and `user` = '".$user_id."'")->fetch_assoc();
...
$Me = $mysqli->query("SELECT * FROM users WHERE `id` = '".$user_id."'")->fetch_assoc();
if($pMY > 0 and $pVS > 0) {
...
elseif(isset($PokM['type']) && $PokM['type'] == 'champion') {$type1 = 'poketitleshine';  $img1 = 'shine';  $name1 = 'champion';}
...
elseif(isset($PokV['type']) && $PokV['type'] == 'champion') {$type2 = 'poketitleshine';  $img2 = 'shine';  $name2 = 'champion';}
...
if(isset($_POST['i_id']) and $aMY == 0 and $_POST['i_id'] > 0) {
    ...
    $PokM = $mysqli->query("SELECT * FROM user_pokemons WHERE `id` = '".$pMY."' and `user_id` = '".$user_id."'")->fetch_assoc();
    if( ($ctg['categories'] == 2 OR $ctg['categories'] == 4 OR $ctg['categories'] == 5 OR $ctg['categories'] == 21)  and   $inM < 6) { // use Item
    ...
    elseif($ctg['categories'] == 3 and $battle['tip'] == 1 and $PokV['hp'] > 0 and $PokV['catch'] == 0){ // ball
...
if(isset($_POST['pid']) and $aMY == 0 and $_POST['pid'] > 0) {
    ...
    $tPid = $mysqli->query("SELECT * FROM user_pokemons WHERE `id` = '".$pid."' and `id` != '".$pMY."' and `user_id` = '".$mID."' and `hp` > '0' and `active` = '1' ")->fetch_assoc();
    if($tPid) {
        if($PokM['hp'] > 0) {
...
if(array_key_exists('type',$_GET) && $_GET['type'] == "atk" and $aMY == 0 and $PokM['hp'] > 0 and $PokV['hp'] > 0) {
    if($_GET['atnum'] == 1 or $_GET['atnum'] == 2 or $_GET['atnum'] == 3 or $_GET['atnum'] == 4) {
        ...
        //  $infAtk = $mysqli->query("SELECT * FROM attack_my_pokemons WHERE pok_id = ".$PokM['id']." AND $wh != '0'")->fetch_assoc();
        ...
        }
        {
            ...
            if($PokM[$wh] > 0) {
                if($PokM[$wh2] > 0) {
                    ...
                    if($PokM[$wh] == 1015 and $sta2 == 13){ $PokM[$wh] = 10006; }
...
$mysqli->query("UPDATE user_pokemons SET basenum = '319' WHERE `basenum` = '3319' and `user_id` = '".$mID."'");                                  
            ...
            if($battle['kom'] == 0) {
                ...
                if($battle['arena'] == 1) {
                    $mysqli->query("UPDATE `users` SET `jeton` = `jeton`+'1' WHERE `id` = '".$mID."'");
                    ...
                    $mysqli->query("UPDATE `users` SET `barena` = '0' WHERE `id` = '".$vID."'");
                    $mysqli->query("UPDATE `users` SET `barena` = '0' WHERE `id` = '".$mID."'");
                }
...
if($battle['arena'] == 1){
  $mysqli->query("UPDATE `users` SET `jeton` = `jeton`+'1' WHERE `id` = '".$vID."'");
...
if($baseloc['rang'] == 0){
    ...
    $allPkUS = $mysqli->query("SELECT * FROM `user_pokemons` WHERE `user_id`='".$mID."' and `active`='1'");
...
if($battle['tip'] == 0 and $battle['kom'] == 0){
...
?>
        ...
        if($sta->num_rows > 0) {
            while($st = $sta->fetch_assoc()){
                if($st['status'] == 7) { echo "Проклят"."<br>"; }
                ...
                if($st['status'] == 8) { echo "Напуган"."<br>"; }
                ...
                if($st['status'] == 13) { echo "Токсичное отравление"."<br>"; }
...
<?php if($cmp_ > 0){ ?>
    <span id=txt>Смена покемона:
        <select size=1 name=pid>
            ...
            <?php 
            $PkUserZM = $mysqli->query('SELECT * FROM user_pokemons WHERE user_id = '.$user_id.' and active = 1  and hp > 0 and id !='.$pMY);
    ...
    echo "<h2>Бой окончен! Победил ".$Vs["login"]."!</h2><br><a href='game.php?fun=fight&iloss=1'><< уйти</a>";
            ...
            $four = $mysqli->query('SELECT * FROM base_attacks WHERE atac_id = '.$PokM['atk4'])->fetch_assoc();
                ...
                $four = array('attac_name_eng'=>'-','atac_pp'=>0);    
                  ...
                  ?>
                  <?php if($battle['kaptcha'] == 1){ // captcha
 echo "<center><form name='captcha' action='' method='post'><img src='/kp.php'></p>
               <input type='text' name='keystring'><input type='submit' value='Отправить'></form> </center>"; 
           }else{
?>
  <style>
   del {
...
while($itmUsers = $itmUser->fetch_assoc()){
  $itm = $mysqli->query("SELECT * FROM base_items WHERE  `id` = '".$itmUsers['item_id']."'")->fetch_assoc();
    if($itm['use'] == 1){
  ?>
...
?>
<center><b><sup>Раунд: <?=$battle['turn'];?> - Погода: <?=$weather['name']; ?> <?php if($battle['type'] == 1) echo "<font color=red>НАПАДЕНИЕ</font>" ?></sup></b></center>
<br />
<!-- Лог боя-->
<?
        ...
        <?php } ?>
         ...
         <div id=hv class=x>
          ...
          <?php if($st2['status'] == 7) { echo "Проклят"."<br>"; }
                ...
                if($st2['status'] == 8) { echo "Напуган"."<br>"; } 
                ...
                if($st2['status'] == 13) { echo "Toxic"."<br>"; } 
