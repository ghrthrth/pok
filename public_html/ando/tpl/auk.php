<?php
$pok_bd = $mysqli->query('SELECT `id`,`basenum`,`name`,`lvl`,`nn`,`name_new` FROM `user_pokemons` WHERE `user_id` = ' . $_SESSION['user_id'] . ' AND `active` = 1 AND `start_pok_skob` = 0 AND `trade` = 0 AND `master` = ' . $_SESSION['user_id']);
...
if (!empty($_POST['submit_p'])) {
  ...
  $select_user_p = $mysqli->query('SELECT * FROM `users` WHERE `id` = ' . $_SESSION['user_id'])->fetch_assoc();
  ...
  minus_item(10000, 1);
  if ($n_stav < 30000) {
    die("<script>alert('Начальная цена не должна быть ниже 30.000 кредитов!'); location.href='game.php?fun=auk';</script>");
  }
  if (($v_stav < ($n_stav + $hod)) and ($v_stav != 0)) {
    die("<script>alert('Выкуп не может быть меньше суммы начальной ставки и хода!'); location.href='game.php?fun=auk';</script>");
  }
  if ($day < 3) {
    die("<script>alert('Количество дней не должно быть меньше 3!'); location.href='game.php?fun=auk';</script>");
  }
  if ($day > 30) {
    die("<script>alert('Количество дней не должно быть больше 30!'); location.href='game.php?fun=auk';</script>");
  }
  if ($select_user_p['groups'] == 7) {
    die("<script>alert('Арестованным пользователям нельзя выкладывать лоты!'); location.href='game.php?fun=auk';</script>");
  }
  if ($u_pok_count->num_rows == 1) {
    die("<script>alert('У вас в команде должен остаться хотя бы 1 покемон!'); location.href='game.php?fun=auk';</script>");
  }
  if (empty($n_stav) || empty($hod) || empty($day) || empty($pok)) {
    die("<script>alert('Ошибка!'); location.href='game.php?fun=auk';</script>");
  }
  $pok_add = $mysqli->query('SELECT * FROM `user_pokemons` WHERE `id` = ' . $pok . ' AND `user_id` = ' . $_SESSION['user_id'] . ' AND `active` = 1 AND `start_pok_skob` = 0 AND `trade` = 0 AND `master` = ' . $_SESSION['user_id'])->fetch_assoc();
  if ($pok_add) {
...
if (!empty($_POST['submit_i'])) {
  $select_user_i = $mysqli->query('SELECT * FROM `users` WHERE `id` = ' . $_SESSION['user_id'])->fetch_assoc();
  $n_stav = obr_chis($_POST['n_price_i']);
  $v_stav = obr_chis($_POST['v_price_i']);
  $hod = obr_chis($_POST['hod_i']);
  $day = obr_chis($_POST['day_i']);
  $item = obr_chis($_POST['itid']);
  $count = obr_chis($_POST['count_i']);
  if (!item_isset(1, 10000)) {
    die("<script>alert('У вас недостаточно кредитов!'); location.href='game.php?fun=auk';</script>");
  }
  minus_item(10000, 1);
  ...
  if ($select_user_i['groups'] == 7) {
    die("<script>alert('Арестованным пользователям нельзя выкладывать лоты!'); location.href='game.php?fun=auk';</script>");
  }
  if ($item == 999) {
    die("<script>alert('На аукцион нельзя выкладывать яйца покемонов!');location.href='game.php?fun=auk';</script>");
  }
  if ($day > 30) {
    die("<script>alert('Количество дней не должно быть больше 30!'); location.href='game.php?fun=auk';</script>");
  }
  if (empty($n_stav) || empty($hod) || empty($day) || empty($item)) {
    die("<script>alert('Ошибка!'); location.href='game.php?fun=auk';</script>");
  }
  if ($count == 0) {
    die("<script>alert('Укажите количество!'); location.href='game.php?fun=auk';</script>");
  }
  if (item_isset($item, $count)) {
    $bd_it = $mysqli->query('SELECT * FROM `base_items` WHERE `id` = ' . $item)->fetch_assoc();
    $name = '<img src="https://claimbe.ru/img/items/' . $bd_it['id'] . '.gif">' . $bd_it['name'] . ' x' . $count;
    $timer = time() + 3600 * 24 * $day;
    $mysqli->query("INSERT INTO `auk` (`time`,`tipe`,`user`,`user_last`,`name`,`id_lot`,`count`,`n_stav`,`v_stav`,`last_stav`,`hod`) VALUES ('" . $timer . "',2,'" . $_SESSION['user_id'] . "',0,'" . $name . "','" . $item . "','" . $count . "','" . $n_stav . "','" . $v_stav . "',0,'" . $hod . "') ");
    minus_item($count, $item);
    $mysqli->query("INSERT INTO `user_items` (`user_id`,`item_id`,`count`) VALUES (2,'" . $item . "','" . $count . "') ");
  } else {
    die("<script>alert('У вас недостаточное количество предмета!'); location.href='game.php?fun=auk';</script>");
  }
  die("<script>location.href='game.php?fun=auk';</script>");
}
if (!empty($_GET['stavka'])) {
  $select_user_stavka = $mysqli->query('SELECT * FROM `users` WHERE `id` = ' . $_SESSION['user_id'])->fetch_assoc();
    ...
    if ($auk['user'] != $_SESSION['user_id']) {
      if (item_isset(1, $nst)) {
        if ($auk['user_last'] != $_SESSION['user_id']) {
          if ($select_user_stavka['groups'] != 7) {
            $t = time() + 180;
            if ($auk['time'] < $t) {
              $s = $auk['time'] + 180;
              $mysqli->query("UPDATE `auk` SET `time` = '" . $s . "' WHERE `id` = " . $auk['id']);
            }
            if ($auk['last_stav'] != 0) {
            ...
            die("<script>alert('Нельзя участвовать в аукционе арестованным игрокам!'); location.href='game.php?fun=auk';</script>");
          }
        } else {
...
if (!empty($_GET['vikup'])) {
  $select_user_vikup = $mysqli->query('SELECT * FROM `users` WHERE `id` = ' . $_SESSION['user_id'])->fetch_assoc();
  $auk = $mysqli->query('SELECT * FROM `auk` WHERE `id` = ' . $_GET['vikup'])->fetch_assoc();
  if ($auk) {
    if ($auk['v_stav']) {
      if ($auk['user'] != $_SESSION['user_id']) {
        if (item_isset(1, $auk['v_stav'])) {
          if ($select_user_vikup['groups'] != 7) {
            $text_v = "Ваш лот №" . $auk['id'] . " " . $auk['name'] . "  выкупили. Сумма вашего счета пополнена на +" . $auk['v_stav'] . " кр.";
            $date = get_last_online(time());
            if ($auk['tipe'] == 1) {
              ...
              $mysqli->query("INSERT INTO `sends` (`user_id`, `user_to`, `text`,`subject`,`date`) VALUES ('2','" . $auk['user'] . "','" . $text_v . "','Аукцион','" . $date . "')");
              $mysqli->query("INSERT INTO `toast` (`user`, `type`, `head`,`text`) VALUES ('" . $auk['user'] . "','info','Аукцион','Ваш лот был выкуплен!')");
            } else {
              plus_item($auk['count'], $auk['id_lot']);
              $mysqli->query("INSERT INTO `sends` (`user_id`, `user_to`, `text`,`subject`,`date`) VALUES ('2','" . $auk['user'] . "','" . $text_v . "','Аукцион','" . $date . "')");
              $mysqli->query("INSERT INTO `toast` (`user`, `type`, `head`,`text`) VALUES ('" . $auk['user'] . "','info','Аукцион','Ваш лот был выкуплен!')");
            }
...
<center>
  <table width="700px">
    <tr>
      ...
      <td width="50%" style="vertical-align: initial;">
        <h2>Выставить на аукцион предмет</h2>
        <form action='' method='POST'>
          <label>Начальная цена</label><br>
          ...
          <select name='itid'>
            <? while ($it = $item_bd->fetch_assoc()) {
              echo "<option value='" . $it['item_id'] . "'>" . $it['name'] . "</option>";
            } ?>
          </select><br>
          <label>Количество</label><br>
          <input placeholder="Количество" name='count_i' type='text'><br><br>
  ...
  <h1>Список лотов</h1>
  <?
  $lot_bd = $mysqli->query('SELECT * FROM `auk` ORDER BY  `time` ASC, `vip` ASC');
  ?>
  <table id="lot_list">
    <thead>
      <tr>
    ...
    <tbody>
      <? while ($lot = $lot_bd->fetch_assoc()) {
        ...
        if ($lot['v_stav'] == 0) {
          ...
          $vik = "<button  onclick='v_stav(" . $lot['id'] . ")'>Выкупить за " . formatnum($lot['v_stav']) . "</button>";
        }
        if ($lot['last_stav'] == 0) {
          $nst = $lot['n_stav'];
          $stav = $lot['n_stav'];
          ...
          $u = "от <a href='/game.php?fun=treninf&to_id=" . $lot['user_last'] . "' onclick=\"window.open('/game.php?fun=treninf&to_id=" . $lot['user_last'] . "', 'treninf', 'fullscreen=no,scrollbars=yes,width=700,height=560'); return false;\"><IMG SRC='img/question.gif' border=0></a> <font color='" . colorsUsers(infUsr($lot['user_last'], 'groups')) . "'>" . infUsr($lot["user_last"], 'login') . "</font>";
        }
      ?>
        <tr>
          <td>№<?= $lot['id']; ?></td>
          <td><?= $lot['name']; ?></td>
          <td>от <a href='/game.php?fun=treninf&to_id=<?= $lot['user']; ?>' onclick="window.open('/game.php?fun=treninf&to_id=<?= $lot['user']; ?>', 'treninf', 'fullscreen=no,scrollbars=yes,width=700,height=560'); return false;"><IMG SRC='img/question.gif' border=0></a>
            <font color='<?= colorsUsers(infUsr($lot["user"], "groups")); ?>'><?= infUsr($lot["user"], "login"); ?></font>
          </td>
          <td><?= formatnum($nst); ?> <?= $u; ?></td>
          <td><?= formatnum($lot['hod']); ?></td>
          <td>Лот активен до <?= Date("H:i d.m", $lot['time']); ?></td>
          <td><button onclick="n_stav(<?= $lot['id']; ?>)">Сделать ставку <?= formatnum($stav); ?></button></td>
          <td><?= $vik; ?></td>
        </tr>
      <?
...
<ol>
  ...
  <li>Внимание, действует трехминутная отсрочка закрытия между ставками.</li>
</ol>