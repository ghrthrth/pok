		...
		<table width='100%'>
			...
			<tr>
				<td valign='top' align='center' width='255'>
					<img src='<?= HOME_URL ?>/img/pkmn/<?= $img; ?>/<?= numbPok($id); ?>.jpg' width=250 height=190 border=1>
					<br>
					<? if (!isset($_GET['shine'])) { ?>
						<b>обычный</b> -
					...
					<? } else { ?>
						...
						<b>шайни</b>
					<? } ?>
					<div title='Категория силы: <?= $pok['power_category']; ?>' style='width:255; height:18; text-align:left; margin-top:-210'>&nbsp;&nbsp;<? for ($a = 0; $a < $pok['power_category']; $a++) { ?><img src="<?= HOME_URL ?>/img/star.png" border='0'><? } ?></div>
					<div style='width:255; height:30; text-align:right; margin-top:160'>
						<? if (empty($pok['type']) || $pok['type'] == 'not') {
						} else { ?>
							<img src='<?= HOME_URL ?>/img/types/<?= $pok['type']; ?>.gif' title='Тип - <?= atktip($pok['type']); ?>'>
						<? } ?>
						<? if (empty($pok['type_two']) || $pok['type_two'] == 'not') {
						} else { ?>
							<img src='<?= HOME_URL ?>/img/types/<?= $pok['type_two']; ?>.gif' title='Тип - <?= atktip($pok['type_two']); ?>'>&nbsp;&nbsp;
						<? } ?>
					</div>
				</td>
				<td valign='top'>
					...
					<b><img src='<?= HOME_URL ?>/img/sex_0.gif'> / <img src='<?= HOME_URL ?>/img/sex_1.gif'> :</b> <?= $pok['sex_m']; ?>/<?= $pok['sex_f']; ?><br>
					<b>Редкость:</b> 1<br>
							...
							<table>
								...
								?>
									<tr>
										...
										<td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?= $pok_attack['atac_id']; ?>','at','width=500,height=230');><img src="<?= HOME_URL ?>/img/question.gif" alt='Инфо' border='0'></a> <?= $name['attac_name_eng']; ?></td>
									</tr>
								<? } ?>
							</table>
						</td>
								...
								?>
									<tr>
										...
										<td><a HREF=javascript: onClick=win1=window.open('at_view.php?AttackID=<?= $tms["attack"]; ?>','at','width=500,height=230');>
												<img src=<?= HOME_URL ?>/img/question.gif alt=Инфо border=0></a> <?= $nameN['attac_name_eng']; ?></td>
									</tr>
								<?php } ?>
