<?php
if(isset($_GET['p_id']) OR isset($_GET['trn'])){
    $pid = obr_chis($_GET['p_id']);
    $trn = obr_chis($_GET['trn']);
    $inf = $mysqli->query('SELECT * FROM user_pokemons WHERE basenum = '. $pid . ' AND user_id = ' . $trn . '')->fetch_assoc();
    if($inf){
...
if (isset($_GET['police_id']) && isset($_GET['groups']) && $_GET['groups'] == $user['groups']) {
    ...
    if ($inf) {
        ...
        if ($inf['user_id'] == $user_id) {// OR $inf['user_id'] == $user_id) {
            include('ando/functions/tip.functions.php');
            if ($inf['type'] == 'normal') {
                $type = 'poketitle';
                $img = 'normal';
