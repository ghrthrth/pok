...
<BODY>
  <h2>Покемоны на локации <?=$loc['name'];?>:</h2>
<?
if($pok_bd->num_rows >= 1){
  while($pk = $pok_bd->fetch_assoc()){
    ...
    $bd_drop = $mysqli->query('SELECT * FROM `drop_item` WHERE `pok` = '.$pk['pok'].' AND `loc` = '.$loc['id'])->fetch_assoc();
    ...
    ?>
<p style="font-size: 12px"><img src="img/pkmna/<?=numbPok($pk['pok']);?>.gif" border="0"><a href="pokedex.php?sp_id=<?=numbPok($pk['pok']);?>">#<?=numbPok($pk['pok']);?> <?=$bd['name'];?> </a> <?=$pk['lvl'];?>-lvl - <?=$t;?>, <?=$r;?> <?=$d;?></p>
    <?
  }
}else{
